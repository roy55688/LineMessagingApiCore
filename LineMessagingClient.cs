...
namespace Line.Messaging
{
    ...
    public class LineMessagingClient : ILineMessagingClient, IDisposable
    {
        ...
        private const string DEFAULT_URIBIGDATA = "https://api-data.line.me/v2";

        private HttpClient _client;
        private JsonSerializerSettings _jsonSerializerSettings;
        private string _uri;
        private string _uriBigData;
        ...
        public LineMessagingClient(string channelAccessToken, string uri = DEFAULT_URI,string uriBigData = DEFAULT_URIBIGDATA)
        {
...
$@"{{ 
    ...
    ""messages"" : [{string.Join(", ", messages)}] 
        ...
        {
            var request = new HttpRequestMessage(HttpMethod.Get, $"{_uriBigData}/bot/message/{messageId}/content");
            var response = await _client.SendAsync(request).ConfigureAwait(false);
            await response.EnsureSuccessStatusCodeAsync().ConfigureAwait(false);
